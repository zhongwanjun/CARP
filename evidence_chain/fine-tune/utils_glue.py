        ...
        for js in tqdm(data, desc='preparing Evidence chain ranker dataset..'):
            ...
            for table_block in table_blocks:
                for idx,chain in enumerate(table_block['evidence_chain']['positive']):
                    ...
                    examples.append(
                        InputExample(guid=guid, text_a=question+' '+table_block['context'], text_b=positive_chain, label='1'))
                    guid+=1
                    if idx!=len(table_block['evidence_chain']['positive'])-1:
                        negative_chain = convert_chain_to_string(table_block['evidence_chain']['negative'][idx])
                        examples.append(
                            InputExample(guid=guid, text_a=question+' '+table_block['context'], text_b=negative_chain, label='0'))
                        guid+=1
        logger.info(f"Total instances {len(examples)}")
        return examples
    def processing_data_multineg(self,data_path):
        logger.info(f"Loading data from {data_path}")
        with open(data_path, 'r') as f:
            data = json.load(f)#[:100]
        # self.data = self.data[:300]
        ...
        for js in tqdm(data, desc='preparing Evidence chain ranker dataset..'):
            ...
            for table_block in table_blocks:
                for idx,chain in enumerate(table_block['evidence_chain']['positive']):
                    ...
                    guid+=1
                    if idx!=len(table_block['evidence_chain']['positive'])-1:
                        if table_block['evidence_chain']['negative'][idx]:
                            # for negchain in table_block['evidence_chain']['es_negative'][idx]:
                            num = min(len(table_block['evidence_chain']['negative'][idx]),5)
                            for negchain in table_block['evidence_chain']['negative'][idx][:num]:
                                negative_chain = convert_chain_to_string(negchain)
                                examples.append(
                                    InputExample(guid=guid, text_a=question+' '+table_block['context'], text_b=negative_chain, label='0'))
                                guid+=1
        logger.info(f"Total instances {len(examples)}")
        ...
        for js in tqdm(data, desc='preparing evidence chain dataset for ranking candidate paths..'):
            ...
            for tbid,table_block in enumerate(table_blocks):
                for idx, chain in enumerate(table_block['candidate_evidence_chains']):
                    ...
                    examples.append(
                        InputExample(guid='{}#{}#{}'.format(js['question_id'],tbid,idx), text_a=question+' '+table_block['context'], text_b=chain_str, label='1'))
...
processors = {
    ...
    'evidence_chain':EvidenceChainProcessor
}
