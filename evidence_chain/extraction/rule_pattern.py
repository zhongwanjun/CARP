...
class RulePattern():
    ...
    def __init__(self,gpu):
        ...
        self.no_use_single_words = ['The', 'She', 'He', 'They', 'It', 'Them', 'Their', 'A', 'On', 'In', 'To', 'Where',
                                    'There']
        self.predictor_ner = Predictor.from_path('https://storage.googleapis.com/allennlp-public-models/ner-elmo.2021-02-12.tar.gz',cuda_device=gpu)
            # "https://s3-us-west-2.amazonaws.com/allennlp/models/ner-model-2018.12.18.tar.gz",cuda_device=0)
        self.predictor_cp = Predictor.from_path('https://storage.googleapis.com/allennlp-public-models/elmo-constituency-parser-2020.02.10.tar.gz',cuda_device=gpu)
    ...
    def found_key_words(self, claims):
        ...
        try:
            ...
            if 'out of memory' in str(e):
                if claims:
                    print(' dealing with out of memory: entities extraction')
                    all_ent_res = []
                    split_num = 4
                    for i in range(split_num):
                        start = int(len(claims)/split_num) * i
                        end = int(len(claims) / split_num) * (i+1) if i<(split_num-1) else len(claims)
                        inputs = [{'sentence': text} for text in claims[start:end]]
                        try:
                            all_ent_res.extend(self.predictor_ner.predict_batch_json(inputs=inputs))
                        except Exception as e:
                            print(e)
                            all_ent_res.extend([[]]*len(inputs))
                else:
                    all_ent_res = []
            else:
                all_ent_res = []
        try:
            ...
            if 'out of memory' in str(e):
                if claims:
                    print(' dealing with out of memory: continuency parsing')
                    all_tokens = []
                    split_num = 4
                    for i in range(split_num):
                        start = int(len(claims)/split_num) * i
                        ...
                        inputs = [{'sentence': text} for text in claims[start:end]]
                        try:
                            all_tokens.extend(self.predictor_cp.predict_batch_json(inputs=inputs))
                        except Exception as e:
                            print(e)
                            all_tokens.extend([[]]*len(inputs))
                else:
                    all_tokens = []
            else:
                all_tokens = []
