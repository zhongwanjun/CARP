...
from transformers import (AutoConfig, AutoModel, AutoTokenizer,
                          RobertaConfig,
                                      ...
                                      AlbertTokenizer,)
from utils_glue import (compute_metrics, convert_examples_to_features,output_pred_results,
                        output_modes, processors,)
from model import RobertaForSequenceClassificationEC
...
MODEL_CLASSES = {
    ...
    'roberta': (RobertaConfig, RobertaForSequenceClassificationEC, RobertaTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx,_ in enumerate(train_iterator):
        ...
        for step, batch in epoch_iterator:
            ...
            inputs = {'a_input_ids':      batch[0],
                      'a_attention_mask': batch[1],
                      'a_token_type_ids': batch[2] if args.model_type in ['bert', 'xlnet'] else None,  # XLM don't use segment_ids
                      'pos_b_input_ids': batch[3],
                      'pos_b_attention_mask': batch[4],
                      'pos_b_token_type_ids': batch[5] if args.model_type in ['bert', 'xlnet'] else None,
                      # XLM don't use segment_ids
                      'neg_b_input_ids': batch[6],
                      'neg_b_attention_mask': batch[7],
                      'neg_b_token_type_ids': batch[8] if args.model_type in ['bert', 'xlnet'] else None,
                      # XLM don't use segment_ids
                      'labels':         batch[9]}
            ouputs = model(**inputs)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                inputs = {'a_input_ids': batch[0],
                          'a_attention_mask': batch[1],
                          'a_token_type_ids': batch[2] if args.model_type in ['bert', 'xlnet'] else None,
                          # XLM don't use segment_ids
                          'pos_b_input_ids': batch[3],
                          'pos_b_attention_mask': batch[4],
                          'pos_b_token_type_ids': batch[5] if args.model_type in ['bert', 'xlnet'] else None,
                ...
                tmp_eval_loss, reps = outputs[:2]
                eval_loss += tmp_eval_loss.mean().item()
                q,c,neg_c = reps[0],reps[1],reps[2]
        ...
        if(mode=='dev'):
            ...
            with open(output_test_file, "w") as writer:
                ...
                instances = output_pred_results(instances,all_logits)
...
def load_and_cache_examples(args, task, tokenizer, ttype='train'):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(examples, label_list, args.max_seq_length, tokenizer, output_mode,args.max_part_length,
    ...
    all_a_input_ids = torch.tensor([f.a_input_ids for f in features], dtype=torch.long)
    all_a_input_mask = torch.tensor([f.a_input_mask for f in features], dtype=torch.long)
    all_a_segment_ids = torch.tensor([f.a_segment_ids for f in features], dtype=torch.long)
    all_pos_b_input_ids = torch.tensor([f.pos_b_input_ids for f in features], dtype=torch.long)
    all_pos_b_input_mask = torch.tensor([f.pos_b_input_mask for f in features], dtype=torch.long)
    all_pos_b_segment_ids = torch.tensor([f.pos_b_segment_ids for f in features], dtype=torch.long)
    all_neg_b_input_ids = torch.tensor([f.neg_b_input_ids for f in features], dtype=torch.long)
    all_neg_b_input_mask = torch.tensor([f.neg_b_input_mask for f in features], dtype=torch.long)
    all_neg_b_segment_ids = torch.tensor([f.neg_b_segment_ids for f in features], dtype=torch.long)
    all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)
    # all_mask_idx = torch.tensor([f.mask_idx for f in features], dtype=torch.long)
    dataset = TensorDataset(all_a_input_ids, all_a_input_mask, all_a_segment_ids,
                            all_pos_b_input_ids, all_pos_b_input_mask, all_pos_b_segment_ids,
                            all_neg_b_input_ids, all_neg_b_input_mask, all_neg_b_segment_ids,
                            all_label_ids)
...
def main():
    ...
    parser.add_argument("--max_part_length", default=248, type=int,
                        help="max length of each evidence node")
