...
sys.path.append('../')
from qa.qa_model import LongformerQAModelEC
# from transformers.data.metrics.squad_metrics import compute_predictions_logits
from qa.utils_qa import compute_predictions_logits, normalize_answer, compute_f1, compute_exact

# from utils import readGZip
from qa.qa_dataset_ori import prepare_qa_data, OTTQAProcessor, OTTQAFeatures, ottqa_convert_examples_to_features, \
...
MODEL_CLASSES = {
    ...
    'longformer': (LongformerConfig, LongformerQAModelEC, LongformerTokenizer),
    ...
    for epoch in train_iterator:
        ...
        if args.local_rank in [-1, 0]:
            if args.local_rank == -1 and args.do_eval and args.evaluate_during_training:
                predictions, nbests = evaluate_simplified(dev_dataset, dev_examples, dev_features, args, model,
                                                          tokenizer, prefix='dev')
...
# def test(dataset, examples, features, args, model, tokenizer, prefix=""):
def test(args, model, tokenizer, prefix=""):
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)

    test_dataset, test_examples, test_features, qid2idx = load_and_cache_examples(args, tokenizer, output_examples=True,
...
def main():
    ...
    special_tokens_dict = {'additional_special_tokens': ["[TO]","[QUESTION]","[EVIDENCE_CHAIN]","[END_EVIDENCE_CHAIN]","[TAB]","[HEADER]", "[PASSAGE]", "[TB]","[DATA]","[TITLE]","[SECTITLE]","[SEP]"]}
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        dev_dataset, dev_examples, dev_features = load_and_cache_examples(args, tokenizer, output_examples=True,
                                                                          evaluate=True)
        predictions, nbests = evaluate_simplified(dev_dataset, dev_examples, dev_features, args, model, tokenizer,
                                                  prefix='eval')
        results = get_raw_scores_finer(dev_examples, nbests)
        # results = get_raw_scores(dev_examples, nbests)
